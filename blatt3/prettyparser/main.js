...
// Requires: npm install antlr4
// Generated parser files must live in ./js-gen (PrettyParserLexer.js, PrettyParserParser.js)
...
// indent a multi-line string with baseIndent for each non-empty line
function applyIndent(text, indentLevel) {
    const base = ' '.repeat(indentLevel * 4);
    return text
        .split('\n')
        .map(ln => (ln === '' ? '' : base + ln))
        .join('\n');
